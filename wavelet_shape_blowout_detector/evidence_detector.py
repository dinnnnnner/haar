...
class EvidenceConfig:
    ...
    min_candidate_gain: float = 0.0060
    min_trigger_slope: float = 0.0033
    max_physical_gain: float = 0.035
    candidate_timeout_frames: int = 80
...
class _WheelEvidence:
    ...
    recent_peak_haar: float = 0.0
    recent_peak_slope: float = 0.0
...
class EvidenceBlowoutDetector:
    ...
    def _advance_state(
        ...
        if state.rise_evidence == 0.0:
            state.recent_peak_haar = 0.0
            state.recent_peak_slope = 0.0
        else:
            state.recent_peak_haar = max(haar, 0.95 * state.recent_peak_haar)
            state.recent_peak_slope = max(slope, 0.95 * state.recent_peak_slope)
        ...
        if state.phase in {"warming", "normal"}:
            ...
            if (
                ...
                and state.recent_peak_slope >= self.cfg.min_trigger_slope
            ):

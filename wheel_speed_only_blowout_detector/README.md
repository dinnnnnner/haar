...
时才锁存报警。默认先观察 0.55 秒；只有四轮几何平均速度变化较平稳时允许
早确认，否则继续观察到 0.7 秒并使用原确认门限。确认期还会检查同对角另一轮
没有强烈反向变化。
...
- 8 条真实 RR 爆胎基线样本：8/8 正确检出，无提前或错误轮位报警；平均确认
  延迟 0.677 秒，最大 0.70 秒。
- 488 条增强测试：408/408 条事件在 2 秒内正确检出；80/80 条正常窗口无误报，
  平均确认延迟 0.679 秒、最大 0.718 秒。
...
[`validation_summary.json`](validation_summary.json)。两套轮速算法统一的优化前后
评估可用仓库根目录的 `evaluate_speed_algorithms.py` 复跑，结果写入
`speed_algorithm_evaluation/`。

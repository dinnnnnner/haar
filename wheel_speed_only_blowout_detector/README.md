...
前后轴共同差在 `d` 中消失。算法只有在两项同时出现正向边沿并持续高于基线
时才锁存报警。默认先观察 0.55 秒；平稳工况使用严格的共同速度门限，净减速
占主要变化时才切换到 0818 Brk 标定的制动门限，否则继续观察到 0.7 秒。
确认期还会检查同对角另一轮和对角持续证据，排除普通制动瞬态。
...
这个历史控制台包含旧的 8 条爆胎回放和 37 条正常道路记录，也可以在“打开自己的 CSV”
...
- 0818 的 3 条有效 RR 爆胎（60kpa、Acc、Brk）：3/3 正确检出，无提前或错误
  轮位报警；平均确认延迟 0.843 秒，最大 0.94 秒。
- 37 条真实正常道路记录：8,922,100 帧、24.784 小时，0/37 记录误报。

正样本优化只以这三条 0818 数据为准；ly 爆胎数据已排除，40kph 的标注和轮速
反馈问题暂缓。参数使用过同源数据开发，以上不是锁参后的独立盲测；FL、FR、
RL 和多轮实车爆胎仍需补充。逐项汇总见
[`validation_summary.json`](validation_summary.json)，统一复跑使用仓库根目录的
`evaluate_0818_algorithms.py`，结果写入 `0818_algorithm_evaluation/`。

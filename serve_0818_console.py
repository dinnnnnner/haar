...
DEFAULT_0821_INPUT_DIR = WORKSPACE_ROOT / "0821"
DEFAULT_0821_EVALUATION = WORKSPACE_ROOT / "0821_quant_evaluation" / "summary.json"
DEFAULT_0826_INPUT_DIR = WORKSPACE_ROOT / "0826"
...
class ConsoleState:
    def __init__(
        ...
        *,
        ...
        input_0821_dir: Path | None = None,
        evaluation_0821: Path | None = None,
        input_0826_dir: Path | None = None,
        ...
        self.input_0821_dir = (
            None if input_0821_dir is None else input_0821_dir.resolve()
        )
        self.evaluation_0821 = (
            None if evaluation_0821 is None else evaluation_0821.resolve()
        )
        self.input_0826_dir = (
            None if input_0826_dir is None else input_0826_dir.resolve()
        )
        self.evaluation_0826 = (
            None if evaluation_0826 is None else evaluation_0826.resolve()
        )
        self.input_0827_dir = (
            None if input_0827_dir is None else input_0827_dir.resolve()
        )
        ...
        paths_0819 = (
            []
            if self.input_0819_dir is None
            else sorted(self.input_0819_dir.glob("*.txt"))
        )
        if self.input_0819_dir is not None and not paths_0819:
            raise ValueError(f"目录内没有 0819 txt 数据：{self.input_0819_dir}")
        self.case_ids_0819 = [path.stem for path in paths_0819]
        self.paths_0819 = {path.stem: path for path in paths_0819}
        ...
        self.cases_0821 = self._load_raw_evaluation_cases(
            self.input_0821_dir, self.evaluation_0821, "0821"
        )
        self.case_ids_0821 = list(self.cases_0821)
        self.cases_0826 = self._load_raw_evaluation_cases(
            self.input_0826_dir, self.evaluation_0826, "0826"
        )
        self.case_ids_0826 = list(self.cases_0826)
        self.cases_0827 = self._load_raw_evaluation_cases(
    ...
    def _load_0820_cases(self) -> dict[str, RawEvaluationCase]:
        return self._load_raw_evaluation_cases(
            self.input_0820_dir, self.evaluation_0820, "0820"
        )

    @staticmethod
    def _load_raw_evaluation_cases(
        input_dir: Path | None,
        evaluation: Path | None,
        dataset: str,
    ) -> dict[str, RawEvaluationCase]:
        if input_dir is None or evaluation is None:
            return {}
        if not evaluation.is_file():
            raise FileNotFoundError(f"{dataset} 评价摘要不存在：{evaluation}")
        payload = json.loads(evaluation.read_text(encoding="utf-8"))
        if payload.get("schema_version") != 1:
            raise ValueError(f"{dataset} 评价摘要版本不受支持，请重新运行评价")
        cases: dict[str, RawEvaluationCase] = {}
        for row in payload.get("cases", []):
            input_path = input_dir / row["input_file"]
            if not input_path.is_file():
                raise FileNotFoundError(f"{dataset} 原始记录不存在：{input_path}")
            if input_path.stat().st_size != row.get("input_size"):
                raise ValueError(
                    f"{dataset} 评价摘要已过期，请重新运行评价：{input_path.name}"
                )
            case_id = row["case"]
    ...
    def _analyze_0819(self, case_id: str) -> CaseAnalysis:
        if case_id not in self.paths_0819:
            raise KeyError(case_id)
        return analyze_file(self.paths_0819[case_id])

    def analyze_0819(self, case_id: str) -> CaseAnalysis:
        return self._analysis_0819(case_id)
    ...
    def render_index(self, dataset: str = "0818") -> str:
        ...
        if dataset == "0821":
            return self._render_0821_index()
        if dataset == "0826":
...
""",
    ...
    def _render_0819_index(self) -> str:
        if not self.case_ids_0819:
            raise ValueError("未配置 0819 原始数据")
        analyses = [self.analyze_0819(case_id) for case_id in self.case_ids_0819]
        quant_hits = sum(
            any(value is not None for value in data.quant_first_alarms)
            for data in analyses
        )
        ...
        for data in analyses:
            ...
            event = data.signal_event_time_s
            rows.append(
                f"<tr><td><a href='/case/{quote(case_id)}?dataset=0819'>{html.escape(case_id)}</a>"
                f"<small class='cell-sub'>{html.escape(data.input_path.name)}</small></td>"
                f"<td>{len(data.times):,}</td><td>{data.duration_s:.2f}s</td>"
                f"<td>{'—' if event is None else f'{event:.2f}s'}</td>"
                f"<td>{html.escape(_alarm_text(data.quant_first_alarms, event))}</td>"
                f"<td><a class='mini-button' href='/case/{quote(case_id)}?dataset=0819'>运行并查看</a></td></tr>"
            )
        return _page(
            "0819 Quant 回放控制台",
            f"""
<header><div><p class='eyebrow'>0819 · QUANT REPLAY</p><h1>0819 Quant 回放控制台</h1>
<p class='muted'>原始齿信号重建 · quant · 四轮轮速 · 完整因果回放</p></div>
<nav><a class='button' href='/summary.json?dataset=0819'>下载回放摘要</a></nav></header>
{self._dataset_tabs('0819')}
<section class='cards'>
 <div class='card accent'><span>0819 记录</span><strong>{len(analyses)}</strong><small>{sum(len(data.times) for data in analyses):,} 帧</small></div>
 <div class='card'><span>quant 报警</span><strong>{quant_hits}/{len(analyses)}</strong><small>当前默认参数</small></div>
 <div class='card'><span>持续爆胎信号</span><strong>0/{len(analyses)}</strong><small>当前文件均未标注爆胎事件</small></div>
</section>
<section class='panel'><div class='controls'><input id='search' placeholder='搜索 0819 记录…'><span id='count'></span></div>
<div class='table-wrap'><table><thead><tr><th>记录</th><th>帧数</th><th>时长</th><th>信号时刻</th><th>quant</th><th>操作</th></tr></thead>
<tbody>{''.join(rows)}</tbody></table></div></section>
...
""",
    ...
    def _render_0821_index(self) -> str:
        if not self.cases_0821:
            raise ValueError("未配置 0821 quant 评价结果")
        correct = sum(
            case.quant_first_alarms[3] is not None
            and all(value is None for value in case.quant_first_alarms[:3])
            for case in self.cases_0821.values()
        )
        event_count = sum(
            case.signal_event_time_s is not None for case in self.cases_0821.values()
        )
        rows = []
        for case in self.cases_0821.values():
            event = case.signal_event_time_s
            case_url = f"/case/{quote(case.case_id)}?dataset=0821"
            rows.append(
                f"<tr><td><a href='{case_url}'>{html.escape(case.case_id)}</a>"
                f"<small class='cell-sub'>{html.escape(case.input_path.name)}</small></td>"
                f"<td>{case.frames:,}</td><td>{case.duration_s:.2f}s</td>"
        ...
        total_frames = sum(case.frames for case in self.cases_0821.values())
        total_duration = sum(case.duration_s for case in self.cases_0821.values())
        return _page(
            "0821 Quant 爆胎回放控制台",
            f"""
<header><div><p class='eyebrow'>0821 · QUANT RR BLOWOUT REPLAY</p><h1>0821 Quant 爆胎回放控制台</h1>
<p class='muted'>定速、加速与减速工况 · RR 爆胎真值 · quant 完整因果评价</p></div>
<nav><a class='button' href='/summary.json?dataset=0821'>下载回放摘要</a></nav></header>
{self._dataset_tabs('0821')}
<section class='cards'>
 <div class='card accent'><span>0821 RR 事件</span><strong>{len(self.cases_0821)}</strong><small>{total_frames:,} 帧 / {total_duration / 60:.2f} 分钟</small></div>
 <div class='card'><span>quant RR 正确检出</span><strong>{correct}/{len(self.cases_0821)}</strong><small>当前默认参数</small></div>
 <div class='card'><span>持续爆胎信号</span><strong>{event_count}/{len(self.cases_0821)}</strong><small>按连续 20 帧高电平定位</small></div>
</section>
<section class='panel'><div class='controls'><input id='search' placeholder='搜索 0821 记录…'><span id='count'></span></div>
<div class='table-wrap'><table><thead><tr><th>记录</th><th>帧数</th><th>时长</th><th>RR 真值时刻</th><th>quant / 延迟</th><th>操作</th></tr></thead>
<tbody>{''.join(rows)}</tbody></table></div></section>
<section class='notice'><b>回放口径：</b>红线取原始信号第一段连续至少 20 帧的高电平；报警时间为实际确认帧，延迟不使用回推的候选起点。</section>
<script>{_FILTER_SCRIPT}</script>
""",
        )

    def _render_0826_index(self) -> str:
        if not self.cases_0826:
            raise ValueError("未配置 0826 quant 评价结果")
        correct = sum(
            case.quant_first_alarms[0] is not None
            and all(value is None for value in case.quant_first_alarms[1:])
            for case in self.cases_0826.values()
        )
        event_count = sum(
            case.signal_event_time_s is not None for case in self.cases_0826.values()
        )
        rows = []
        for case in self.cases_0826.values():
            event = case.signal_event_time_s
            case_url = f"/case/{quote(case.case_id)}?dataset=0826"
            relative_path = (
                case.input_path.relative_to(self.input_0826_dir)
                if self.input_0826_dir is not None
        ...
        total_frames = sum(case.frames for case in self.cases_0826.values())
        total_duration = sum(case.duration_s for case in self.cases_0826.values())
        return _page(
            "0826 Quant FL 爆胎回放控制台",
            f"""
<header><div><p class='eyebrow'>0826 · QUANT FL BLOWOUT REPLAY</p><h1>0826 Quant FL 爆胎回放控制台</h1>
<p class='muted'>定速、急加速与急减速工况 · FL 爆胎真值 · quant 完整因果评价</p></div>
<nav><a class='button' href='/summary.json?dataset=0826'>下载回放摘要</a></nav></header>
{self._dataset_tabs('0826')}
<section class='cards'>
 <div class='card accent'><span>0826 FL 事件</span><strong>{len(self.cases_0826)}</strong><small>{total_frames:,} 帧 / {total_duration / 60:.2f} 分钟</small></div>
 <div class='card'><span>quant FL 正确检出</span><strong>{correct}/{len(self.cases_0826)}</strong><small>当前默认参数</small></div>
 <div class='card'><span>持续爆胎信号</span><strong>{event_count}/{len(self.cases_0826)}</strong><small>按连续 20 帧高电平定位</small></div>
</section>
<section class='panel'><div class='controls'><input id='search' placeholder='搜索 0826 记录或工况…'><span id='count'></span></div>
<div class='table-wrap'><table><thead><tr><th>工况 / 记录</th><th>帧数</th><th>时长</th><th>FL 真值时刻</th><th>quant / 延迟</th><th>操作</th></tr></thead>
<tbody>{''.join(rows)}</tbody></table></div></section>
<section class='notice'><b>命名口径：</b>急加速和急减速目录中有同名记录，display 使用“工况 / 文件名”作为唯一 ID，原始文件不改名。</section>
<script>{_FILTER_SCRIPT}</script>
""",
        )

    def _render_0827_index(self) -> str:
        if not self.cases_0827:
            raise ValueError("未配置 0827 quant 评价结果")
        quant_hits = sum(
            any(value is not None for value in case.quant_first_alarms)
            for case in self.cases_0827.values()
        )
        event_count = sum(
            case.signal_event_time_s is not None for case in self.cases_0827.values()
        )
        rows = []
        for case in self.cases_0827.values():
            event = case.signal_event_time_s
            case_url = f"/case/{quote(case.case_id)}?dataset=0827"
            rows.append(
                f"<tr><td><a href='{case_url}'>{html.escape(case.case_id)}</a>"
                f"<small class='cell-sub'>{html.escape(case.input_path.name)}</small></td>"
                f"<td>{case.frames:,}</td><td>{case.duration_s:.2f}s</td>"
        ...
        total_frames = sum(case.frames for case in self.cases_0827.values())
        total_duration = sum(case.duration_s for case in self.cases_0827.values())
        return _page(
            "0827 Quant 工况回放控制台",
            f"""
<header><div><p class='eyebrow'>0827 · QUANT MANEUVER REPLAY</p><h1>0827 Quant 工况回放控制台</h1>
<p class='muted'>ABS、低附着、分离路面、绕桩与越野工况 · quant 完整因果评价</p></div>
<nav><a class='button' href='/summary.json?dataset=0827'>下载回放摘要</a></nav></header>
{self._dataset_tabs('0827')}
<section class='cards'>
 <div class='card accent'><span>0827 记录</span><strong>{len(self.cases_0827)}</strong><small>{total_frames:,} 帧 / {total_duration / 60:.2f} 分钟</small></div>
 <div class='card'><span>quant 报警</span><strong>{quant_hits}/{len(self.cases_0827)}</strong><small>当前默认参数</small></div>
 <div class='card'><span>持续爆胎信号</span><strong>{event_count}/{len(self.cases_0827)}</strong><small>按连续 20 帧高电平统计</small></div>
</section>
<section class='panel'><div class='controls'><input id='search' placeholder='搜索 0827 工况…'><span id='count'></span></div>
<div class='table-wrap'><table><thead><tr><th>记录</th><th>帧数</th><th>时长</th><th>信号时刻</th><th>quant</th><th>操作</th></tr></thead>
<tbody>{''.join(rows)}</tbody></table></div></section>
<section class='notice'><b>数据口径：</b>0827 按正常驾驶工况展示；原始信号若没有连续 20 帧高电平，则真值为无爆胎。</section>
<script>{_FILTER_SCRIPT}</script>
""",
        )
...
""",
    ...
    def _dataset_tabs(self, selected: str) -> str:
        ...
        if self.case_ids_0821:
            items.append(("0821", "0821 爆胎数据"))
        if self.case_ids_0826:
    ...
    def summary(self, dataset: str = "0818") -> dict[str, object]:
        if dataset == "0819":
            cases = []
            for case_id in self.case_ids_0819:
                data = self.analyze_0819(case_id)
                cases.append(
                    {
                        "case": case_id,
        ...
        if dataset == "0821":
            return {
                "input_dir": (
                    None if self.input_0821_dir is None else str(self.input_0821_dir)
                ),
                "evaluation": (
                    None if self.evaluation_0821 is None else str(self.evaluation_0821)
                ),
                "cases": [
                    {
                        "case": case.case_id,
                ...
                "cases": [
                    {
                        "case": case.case_id,
                        "input_file": str(
                            case.input_path.relative_to(input_dir)
    ...
    def render_case(
        ...
        if dataset == "0818":
            ...
            if start_s is None and end_s is None:
                start_s = 0.0
                end_s = min(data.times[-1], 15.0)
            data_start = data.times[0]
            data_end = data.times[-1]
            case_title = case_id
            dataset_title = "0819 新采数据"
            truth_title = "持续爆胎信号"
            truth_value = "未标注"
            ...
            case = self.cases_0821.get(case_id)
            if case is None:
                raise KeyError(case_id)
            event = case.signal_event_time_s
            if start_s is None and end_s is None:
            ...
            data_end = case.duration_s
            case_title = case.case_id
            dataset_title = "0821 RR 爆胎数据"
            truth_title = "RR 爆胎真值"
            truth_value = "—" if event is None else f"{event:.2f}s"
            truth_note = "原始信号仍逐帧显示"
            return_url = "/?dataset=0821"
        elif dataset == "0826":
            case = self.cases_0826.get(case_id)
            if case is None:
                raise KeyError(case_id)
            event = case.signal_event_time_s
            if start_s is None and end_s is None:
            ...
            data_end = case.duration_s
            case_title = case.case_id
            dataset_title = "0826 FL 爆胎数据"
            truth_title = "FL 爆胎真值"
            truth_value = "—" if event is None else f"{event:.2f}s"
            truth_note = "原始信号仍逐帧显示"
            return_url = "/?dataset=0826"
        elif dataset == "0827":
            case = self.cases_0827.get(case_id)
            if case is None:
                raise KeyError(case_id)
            event = case.signal_event_time_s
            if start_s is None and end_s is None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-0821-dir", type=Path, default=DEFAULT_0821_INPUT_DIR
    )
    parser.add_argument(
        "--evaluation-0821", type=Path, default=DEFAULT_0821_EVALUATION
    )
    parser.add_argument(
        "--input-0826-dir", type=Path, default=DEFAULT_0826_INPUT_DIR
    )
    parser.add_argument(
        "--evaluation-0826", type=Path, default=DEFAULT_0826_EVALUATION
    )
    parser.add_argument(
        "--input-0827-dir", type=Path, default=DEFAULT_0827_INPUT_DIR
    )
    parser.add_argument(
    ...
    state = ConsoleState(
        ...
        input_0821_dir=args.input_0821_dir,
        evaluation_0821=args.evaluation_0821,
        input_0826_dir=args.input_0826_dir,

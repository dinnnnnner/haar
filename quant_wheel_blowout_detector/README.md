...
冲击触发后默认观察约 0.55 秒。确认还要求物理轮速偏差、高位占比、共同速度
范围，以及确认窗口至少 95% 的帧保持同一轮位指纹。单轮物理投影超过 2.5%
按轮滑或数据异常排除；候选期间冻结在线统计模型，避免把事件吸收到正常基线。
...
机器可读结果见 [`validation_summary.json`](validation_summary.json)。两套轮速算法
统一的优化前后评估可用仓库根目录的 `evaluate_speed_algorithms.py` 复跑，结果
写入 `speed_algorithm_evaluation/`。

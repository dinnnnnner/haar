...
0818 的冲击触发带为 3.0–4.5 标准差，触发后默认观察 16 帧（约 0.16 秒）。确认还要求
持续物理轮速偏差、高位占比、共同速度范围，以及至少 82.5% 的帧保持同一轮位
指纹。三个非目标轮的 Hadamard 反投影中位值必须保持非正，以确认这是纯净的
...
- 0818 的 3 条有效 RR 爆胎（60kpa、Acc、Brk）：3/3 检出，无提前或错误轮位
  报警；延迟分别为 46、41、32 帧，平均 39.7 帧（0.397 秒），最大 46 帧
  （0.46 秒）。
...
反馈问题暂缓。20 帧目标未通过正常道路反例验证：短时单轮冲击与爆胎初段仍有
重叠。阈值使用过同源数据和正常道路反例开发，因此不是锁参后的独立

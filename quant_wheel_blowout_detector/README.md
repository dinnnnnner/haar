...
边沿较弱时还可走受保护触发，要求冲击隔离度至少为 `1.8`、物理边沿至少为
`0.0025`，同时当前持续匹配分和持续隔离度分别达到 `2.5` 和 `1.8`。受保护候选
不能直接使用常规 16 帧确认；它必须通过后面的强物理路径，或收集至少 50 帧
持续证据。若受保护候选期间出现常规触发，则从该帧重新开始常规候选。
...
常规候选期间冻结在线均值和协方差，避免把事件吸收到正常模型。受保护候选的
在线模型继续更新，防止正常道路上的弱冲击长期冻结基线。默认至少收集 16 帧
候选证据。以下任一条件会清除候选：

- 物理平台连续 15 帧低于 `-0.0025`；
- 单轮物理峰值超过 `0.030`，按轮滑或异常冲击处理；
- 候选达到 120 帧仍未确认。
...
- 强物理路径只在候选前 30 帧启用。物理峰值需在 `0.018` 至 `0.025` 之间，最近
  16 帧物理中位值至少为 `0.012`，风险峰值至少为 `60`；允许非目标轮出现小幅
  同向变化，但最大非目标轮中位值不能超过 `0.002`，且不能超过目标轮中位值的
  10%。
- 受保护持续路径至少等待 50 帧。物理峰值至少为 `0.008`，最近 50 帧物理中位值
  至少为 `0.0045`，持续比例至少为 65%，风险中位值至少为 `27`，隔离度中位值
  至少为 `1.5`，隔离比例至少为 75%。同样使用 10% 非目标轮比例限制，共同对数
  轮速范围不能超过 `0.120`。
...
| 受保护触发 | `guarded_min_physical_edge` / `guarded_shock_isolation_z` | 0.0025 / 1.8 |
| 确认 | `confirm_frames` | 16 帧 |
| 受保护确认 | `guarded_confirm_frames` | 50 帧 |
